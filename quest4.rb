...
#1. オブジェクト指向の目的
#↓
#オブジェクト指向プログラミングは、プログラムをより構造化され、管理しやすくし、現実の問題に対処するための強力なツールです。
#初心者にとっても、問題の分解やモデル化を通じてコードを理解しやすくし、コードの再利用と保守性を向上させる手段として役立ちます。
...
#似たようなオブジェクトを複数作る時に、全てのプロパティやメソッドをいちいちプログラミングするのは非常に手間が掛かりますが、
#継承を使うことにより、同じ機能を実装できます。
...
vending_machine = VendingMachine.new(maker)
